...
function LanguageToggle({
  ...
  return (
    <div
      role="group"
      aria-label={label}
      className="grid h-9 w-[88px] grid-cols-2 rounded-full border border-lavender/30 bg-navy-card/70 p-0.5"
    >
      {(
        [
          ['zh-TW', '中', '切換為繁體中文'],
        ...
        <button
          ...
          className={`focus-ring rounded-full font-sans-tc text-[11px] transition-colors ${
            locale === value ? 'bg-gold text-navy-black' : 'text-ink-dark-soft hover:text-ink-dark'
          }`}
        >
          {text}
        </button>
    ...
    <header className="fixed inset-x-0 top-0 z-[9000]">
      <div
        className={`transition-colors duration-500 ${
          scrolled ? 'border-b border-gold/10 bg-navy-black/80 backdrop-blur-md' : 'bg-transparent'
        }`}
      >
        <nav className="mx-auto flex h-20 max-w-6xl items-center justify-between px-5 sm:px-8">
          ...
          >
            ...
            <span className="font-serif-en text-lg tracking-wide text-ink-dark">{siteShared.brand}</span>
          </a>

          <div className="hidden items-center gap-3 md:flex">
            <LanguageToggle locale={locale} setLocale={setLocale} label={copy.common.language} />
            <a
              href="#contact"
              data-cursor-hover
              onClick={(event) => {
        ...
        <div className="fixed inset-0 top-20 z-[8999] flex flex-col bg-navy-black/92 px-8 py-10 backdrop-blur-md md:hidden">
          ...
          <div className="mx-auto flex flex-col items-center gap-5">
            ...
            <a
              ...
              className="focus-ring flex items-center gap-2 rounded-full border border-gold/60 px-6 py-3 text-ink-dark"
            >
              {copy.common.connect}
